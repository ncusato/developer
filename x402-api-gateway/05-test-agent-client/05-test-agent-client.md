...
You will run a provided Node.js client that behaves like an AI agent. It calls the paid API, receives a 402 challenge, signs a payment authorization with a Base Sepolia wallet, retries the request, and prints paid market signal data.

### Objectives

- Prepare a testnet wallet for the x402 payment.
- Generate the Node.js agent client from workshop assets.
- Pay for one market signal query through API Gateway.
...
2. Confirm the output shows:
    ...
    - `Paid status: 200`.
    - The number of returned market signal rows.
